...
Bash is ONLY for: `git`, `mkdir`, `rm`, `mv`, `cd`, `ls`, `npm install`, `pip install`, and
...
The raw file content stays in the sandbox.
...
2. **FOLLOW-UP**: `ctx_search(queries: ["q1", "q2", ...])` — Query indexed content.
   Pass ALL questions as array in ONE call.
...
### Fallback — ctx tools unavailable

If the `ctx_*` MCP tools cannot be loaded (server not connected; ToolSearch finds nothing),
the routing rules still apply in spirit. In that case:

- **Delegate, don't dump**: run whole-repo scans, reviews, lint/test runs, and multi-file
  analysis in subagents that return compact summaries only (subagents inherit routing
  automatically). The main context never sees raw tool output.
- **Cap raw Bash**: any command whose output must enter context is piped through
  `head`/`tail`/`wc -l`/`--stat`/`jq` so it stays under ~20 lines. Append `# ctx-ok`
  (with a leading space) to acknowledge the hook.
- **Spill to scratchpad**: redirect large output (diffs, API responses, logs) to a
  scratchpad file, then read only the slices needed.
- Never paste full diffs, logs, or file listings into context — summarize instead.
