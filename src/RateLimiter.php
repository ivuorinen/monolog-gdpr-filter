...
class RateLimiter
{
    ...
    public function __construct(
        ...
        if ($this->maxRequests <= 0) {
            throw new InvalidArgumentException('Maximum requests must be a positive integer, got: ' . $this->maxRequests);
        }

        if ($this->maxRequests > 1000000) {
            throw new InvalidArgumentException(
                'Maximum requests cannot exceed 1,000,000 for memory safety, got: ' . $this->maxRequests
            );
        }
        ...
        if ($this->windowSeconds <= 0) {
            throw new InvalidArgumentException(
                'Window seconds must be a positive integer, got: ' . $this->windowSeconds
            );
        }

        if ($this->windowSeconds > 86400) { // 24 hours max
            throw new InvalidArgumentException(
                'Window seconds cannot exceed 86,400 (24 hours) for practical reasons, got: ' . $this->windowSeconds
            );
        }
    }
     ...
     * @throws InvalidArgumentException When seconds is invalid
     */
    public static function setCleanupInterval(int $seconds): void
    {
        if ($seconds <= 0) {
            throw new InvalidArgumentException('Cleanup interval must be a positive integer, got: ' . $seconds);
        }
    ...
    {
        if (trim($key) === '') {
            throw new InvalidArgumentException('Rate limiting key cannot be empty');
        }
        ...
        if (preg_match('/[\x00-\x1F\x7F]/', $key)) {
            throw new InvalidArgumentException('Rate limiting key cannot contain control characters');
        }
    }
}
